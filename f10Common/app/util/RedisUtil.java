...
public abstract class RedisUtil {
     ...
     */
    @Util
    public static void clean(String[] redisKeys,String name){
        for(String redisKey:redisKeys){
            Set<String> keySet = Redis.keys(redisKey + "*");
            String[] keys = keySet.toArray(new String[]{});
            Logger.info("开始清除%s的Redis缓存,对应的KEY前缀为%s", name, redisKey);
            Long count = 0L;
            try {
