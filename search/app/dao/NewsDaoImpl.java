    ...
    */
    ...
    public List<String> findNewsIndustryIds(long newsId) {
        ...
        for (Map<String, Object> map : mapList) {
            ...
            if ("P0201".equalsIgnoreCase(type) && code.length() > 0) { //老的分类
                if ("K".equalsIgnoreCase(code)) {
                    induCodeSet.addAll(oldCodeK2NewCodeSet);
                } else {
                    String newCode = oldNewInduCodeMap.get(code);
                    if (StringUtils.isBlank(newCode)) {
                        newCode = oldNewInduCodeMap.get(code.substring(0, 1));
                    }
                    if (StringUtils.isNotBlank(newCode)) {
                        induCodeSet.add(newCode.substring(0, 1)); //只取一级分类
                    } else {
                        Logger.warn("newsId[%d]的旧行业分类[%s](且一级分类也没有)没有找到对应的新行业分类", newsId, code);
                    }
                }
            } else { //新分类
                if (code.length() > 0) {
                    induCodeSet.add(code.substring(0, 1));//只取一级分类
                }
            }
        }
    ...
    public List<String> findReportSourceNames(long repId){
        String sql = SqlLoader.getSqlById("findReportSourceNamesList");
        return DbUtil.queryExtractDbWithHandler(sql, new ColumnListHandler<String>(), repId);
    }

    public List<String> findReportAuthors(long repId) {
        String sql = SqlLoader.getSqlById("findReportAuthors");
        return DbUtil.queryExtractDbWithHandler(sql, new ColumnListHandler<String>(), repId);
    }

    public final static Set<String> oldCodeK2NewCodeSet = Sets.newHashSet("G","H","L","M","N","O","Q","R"); //老分类K, 所对应的新分类
            ...
            if ("P0201".equalsIgnoreCase(type) && code.length() > 0) { //老的分类
                if ("K".equalsIgnoreCase(code)) {
                    ...
                    if (StringUtils.isNotBlank(newCode)) {
                        ...
                        Logger.warn("repId[%d]的旧行业分类[%s](且一级分类也没有)没有找到对应的新行业分类", repId, code);
                    }
                }
            } else { //新分类
                if (code.length() > 0) {
    ...
    @Override
    public boolean newsHasAttach(long newsId) {
        String sql = SqlLoader.getSqlById("newsHasAttach");
        long count = DbUtil.queryCount(sql,newsId);
        //判断新闻是否有附件
        return count>0;
    }
